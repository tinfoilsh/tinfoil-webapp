...
export function injectArtifactPolyfills(html: string): string {
  ...
  if (
    headOpenMatch?.index !== undefined &&
    headCloseMatch?.index !== undefined
  ) {
    ...
    const scriptInHeadMatch = headContent.match(/<script\b/i)
    const insertAt =
      scriptInHeadMatch?.index !== undefined
        ? headStart + scriptInHeadMatch.index
        : headEnd
