...
//
// Full vertical slice E2E — the real thing: pair with the real driver, boot the
...
//
//   ./tinfoil-driver up --auto-approve --origin http://127.0.0.1:3000
//   DRIVER_E2E_FULL=1 TINFOIL_API_KEY=sk-... \
//     [DRIVER_E2E_IMAGE=theo-mac] [DRIVER_E2E_OS=mac] \
//     npx vitest run tests/services/computer-use/e2e.full-slice.test.ts
//
// It picks the first ready image from /status unless DRIVER_E2E_IMAGE is set,
// runs a short, low-risk task, and logs every loop event (the audit trail).

import { createDriverConnection } from '@/services/computer-use/access-token'
import type {
  ChatChunk,
  StreamChat,
...
const RUN = process.env.DRIVER_E2E_FULL === '1' && !!process.env.TINFOIL_API_KEY
const ORIGIN = process.env.DRIVER_E2E_ORIGIN ?? 'http://127.0.0.1:3000'
const BASE = process.env.DRIVER_E2E_BASE ?? 'http://127.0.0.1:8765'
const MODEL = process.env.DRIVER_E2E_MODEL ?? 'kimi-k2-6'
const INFERENCE_URL =
  process.env.DRIVER_E2E_INFERENCE ??
...
d('full-slice E2E (real driver + real VM + real Kimi)', () => {
  it('pairs, boots the VM, and drives a short task to completion', async () => {
    const fetchImpl = originFetch(ORIGIN)
    const probe = new DriverClient({ baseUrl: BASE, fetchImpl })

    const status = await probe.getStatus()
    const image =
      process.env.DRIVER_E2E_IMAGE ?? status.images.find((i) => i.ready)?.name
    expect(image, 'need a ready image (set DRIVER_E2E_IMAGE)').toBeTruthy()

    const { refreshCredential } = await runPairing(probe, {
      pollIntervalMs: 300,
      timeoutMs: 20_000,
    })
    const { client, tokens } = createDriverConnection({
      refreshCredential,
      baseUrl: BASE,
      fetchImpl,
    })
    ...
    const manifest: CapabilityManifest = {
      version: 1,
      session: {
        os: (process.env.DRIVER_E2E_OS as GuestOS) ?? 'mac',
        image: image!,
        clone: true,
        idle_timeout: '10m',
      },
