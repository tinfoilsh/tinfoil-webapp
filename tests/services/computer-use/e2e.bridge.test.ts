...
// real CORS gate we care about is the driver's, exercised via the injected
// Origin header below.
/**
 * Bridge E2E — runs my real `DriverClient` against a REAL `tinfoil-driver`.
 ...
 *
 * Gated: skipped unless DRIVER_E2E=1, so normal test runs don't reach for
 * localhost. Run it with the driver up in auto-approve mode:
 *
 *   ./tinfoil-driver up --auto-approve --origin http://localhost:3000
 *   DRIVER_E2E=1 npx vitest run tests/services/computer-use/e2e.bridge.test.ts
 *
 * The browser normally sets `Origin` automatically; here we inject it (Node lets
 * us) so the driver's exact-origin CORS gate is exercised authentically.
 */
...
const RUN = process.env.DRIVER_E2E === '1'
...
d('bridge E2E (real driver)', () => {
  ...
  it('pairs (auto-approve) and mints an access token', async () => {
    ...
    const { tokens } = createDriverConnection({
      refreshCredential,
      baseUrl: BASE,
      fetchImpl,
    })
  ...
  it('rejects a foreign origin with 403', async () => {
    ...
    await expect(client.getStatus()).rejects.toSatisfy(
      (e: unknown) => e instanceof DriverError && e.status === 403,
    )
  })
  ...
  it('rejects a bogus refresh credential with 401', async () => {
    ...
    await expect(
      ...
      (e: unknown) => e instanceof DriverError && e.status === 401,
    )
  })
})
